...
"""Functional tests for ``pipeformer.internal.templates.inputs``."""
import pytest

from pipeformer.internal.templates import inputs
...
def test_parse_config(name: str):
    ...
    _test = inputs.build(project)


@pytest.mark.parametrize("name", functional_test_utils.vector_names())
def test_generate_template(name: str):
    project = functional_test_utils.populated_config(name)
    expected = functional_test_utils.load_vector_as_template(name, "inputs")

    test = inputs.build(project)
